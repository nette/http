...
test('trailing root dot is ignored, so it cannot dodge host patterns', function () {
	$allow = new UrlValidator(hostAllowlist: ['example.com']);
	Assert::true($allow->allowsWithoutDns('https://example.com./'));

	$block = new UrlValidator(hostBlocklist: ['blocked.example.com']);
	Assert::false($block->allowsWithoutDns('https://blocked.example.com./'));

	$default = new UrlValidator;
	Assert::false($default->allowsWithoutDns('https://example.com../')); // empty label is invalid
	Assert::false($default->allowsWithoutDns('https://./'));
	Assert::false($default->allows('https://192.168.1.1./')); // normalizes to the IP literal, private/public policy still applies
	Assert::same(['1.1.1.1'], $default->getResolvedIPs('https://1.1.1.1./'));
});
