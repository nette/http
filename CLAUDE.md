...
- **`getRemoteHost()` is deprecated** (v3.4.0) — it now always returns `null`. Reverse DNS was slow, unreliable and a privacy footgun. Resolve the hostname yourself from `getRemoteAddress()` if you really need it
...
- **`Max-Age`** (v3.4.0): `setCookie()` emits a `Max-Age` attribute (takes precedence over `expires`, ignores client clock). It now builds the `Set-Cookie` header itself instead of delegating to PHP's `setcookie()`. Passing integer `0` as the expiration is **deprecated** — use `null` for a session cookie
...
#### SSRF Protection Kit (v3.4.0)

Two immutable, framework-optional value objects for guarding server-side URL fetches against SSRF and DNS rebinding.

- **`IPAddress`** (`src/Http/IPAddress.php`) - Immutable IPv4/IPv6 value object
  - Construct from string (throws `Nette\InvalidArgumentException` on invalid), or `IPAddress::tryFrom()` / `IPAddress::isValid()`
  - Class predicates: `isPublic()`, `isPrivate()` (RFC 1918/4193), `isLoopback()`, `isLinkLocal()` (incl. cloud metadata `169.254.169.254`), `isMulticast()`, `isReserved()`
  - Family: `isIPv4()`, `isIPv6()`, `isIPv4Mapped()`, `toIPv4()` (normalizes `::ffff:a.b.c.d`)
  - CIDR matching: `isInRange('10.0.0.0/8')` (accepts network/prefix or bare IP as implicit /32 or /128); IPv4-mapped IPv6 is normalized to IPv4 for IPv4 ranges
- **`UrlValidator`** (`src/Http/UrlValidator.php`) - Configurable policy guard
  - Constructor policy: `$schemes` (default `['https']`), `$ports` (default `[443]`, `null` = any), `$allowPrivateIps`, `$allowLoopback`, `$allowLinkLocal`, `$allowReserved`, `$allowUserinfo`, `$hostAllowlist`, `$hostBlocklist`
  - Host patterns: exact (`example.com`) or wildcard subdomain (`*.example.com`, any depth but NOT apex). Multicast is always rejected
  - `allows($url)` - full check incl. DNS: every resolved A/AAAA must pass the IP-range policy
  - `allowsWithoutDns($url)` - fast pre-filter, no DNS (e.g. when DNS validation is delegated to the fetch layer)
  - `getResolvedIPs($url)` - returns validated IPs (A first, then AAAA) to pin the connection via `CURLOPT_RESOLVE` and defeat DNS rebinding

#### Helpers (`src/Http/Helpers.php`)
- `Helpers::expirationToSeconds()` (v3.4.0) - single consistent expiration parser for the whole library. Numbers are relative seconds; `DateTimeInterface` and textual strings (`'20 minutes'`, `'2024-01-01'`) resolve as absolute times
- `Helpers::parseQualityList()` (v3.4.0) - parses HTTP quality-value lists (`Accept`, `Accept-Language`, …) into a ranked token map; `Request::detectLanguage()` is built on it
- `Helpers::formatDate()` - RFC 7231 HTTP date format
- `Helpers::StrictCookieName` - constant for the strict same-site cookie (`_nss`; old `STRICT_COOKIE_NAME` deprecated)
...
Requires PHP 8.3 (since v3.4.0). Heavily uses modern features:
